...
from .mail_utils import get_attachment, get_attachments
...
class Message(admin.ModelAdmin):
    def message_link(self, obj):
        ...
        return mark_safe(
            """<a href="%s" onclick="return showAddAnotherPopup(this);">show</a>"""
            % url
        )
    ...
    def re_send(self, request, queryset):
        ...
        for selected_mail in queryset:
            ...
            try:
                ...
                self.message_user(
                    request, "Message %s is already in the queue" % selected_mail.id
                )
    ...
    def get_urls(self):
        ...
        custom_urls = [
            ...
            url(
                "^mail/attachment/(?P<pk>\d+)/(?P<firma>[0-9a-f]{32})/$",
    ...
    def download_view(self, request, pk, firma):
        ...
        response["Content-Disposition"] = "filename={}".format(arx.filename)
        response.write(base64.b64decode(arx.payload))
        return response
...
class QueuedMessage(MessageRelatedModelAdmin):
    ...
    def message_link(self, obj):
        ...
        return mark_safe(
            """<a href="%s" onclick="return showAddAnotherPopup(this);">%s</a>"""
            % (url, obj.message)
        )
    ...
    list_display = (
        ...
        "message_link",
        "message__to_address",
        "message__from_address",
        "message__subject",
        "message__date_created",
        "priority",
        "not_deferred",
    )
